    ...
    <div className="flex flex-col gap-4">
      <div className="flex aspect-square items-center justify-center overflow-hidden rounded-3xl border border-slate-custom-50 bg-cream-50">
        {photos.length > 0 ? (
          ...
          <img src={`${STORAGE}/${photos[active].photo_path}`} alt={animalName} className="h-full w-full object-cover transition" />
        ) : (
      ...
      {photos.length > 1 && (
        <div className="grid grid-cols-4 gap-3">
          {photos.map((photo, i) => (
            ...
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={`${STORAGE}/${photo.photo_path}`} alt={animalName} className="h-full w-full object-cover" />
            </button>
          ))}
        </div>
      )}
