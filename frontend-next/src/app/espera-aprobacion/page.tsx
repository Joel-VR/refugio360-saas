  ...
  return (
    <PublicShell>
      <section className="mx-auto grid max-w-2xl gap-5 px-6 py-16">
        ...
        <p className="text-sm leading-6 text-slate-custom-700">
          {rejected
        ...
        <div className="rounded-lg border border-slate-custom-50 bg-cream-50 p-5 text-sm text-slate-custom-700">
          <p className="font-semibold">{user?.shelter?.name ?? "Albergue en revisión"}</p>
          <p className="mt-1">{user?.email}</p>
        </div>
        <Link href="/perfil" className="w-fit rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-custom-700">
          Ver perfil
        </Link>
      </section>
    </PublicShell>
