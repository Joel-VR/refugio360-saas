  ...
  if (donationId) {
    return (
      <div className="rounded-lg border border-emerald-200 bg-cream-50 p-8 text-center shadow-sm">
        ...
        <p className="mt-3 text-sm text-slate-custom-700">Referencia: <span className="font-mono font-semibold">#{donationId}</span></p>
        <p className="mx-auto mt-4 max-w-md text-sm leading-6 text-slate-custom-700">Será verificada en 24-48 horas. El equipo revisará el comprobante y actualizará el estado.</p>
        <div className="mt-6 flex justify-center gap-3">
          <Link href="/donar" className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-custom-700">Volver a albergues</Link>
          <Link href={`/refugios/${shelter.slug}/transparencia`} className="rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white">Ver transparencia</Link>
        </div>
      </div>
    );
  }
    ...
    <form onSubmit={submit} className="grid gap-5">
      <section className="rounded-lg border border-slate-custom-50 bg-cream-50 p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => { setDonationType("general"); setAnimalId(null); }} className={`rounded-md border px-4 py-3 text-left ${donationType === "general" ? "border-teal-600 bg-teal-50" : "border-slate-custom-50"}`}>
            ...
            <button type="button" onClick={() => setDonationType("specific")} className={`rounded-md border px-4 py-3 text-left ${donationType === "specific" ? "border-teal-600 bg-teal-50" : "border-slate-custom-50"}`}>
          ...
          {donationType === "specific" && (
            <div className="grid gap-3 sm:grid-cols-2">
              {animals.map((animal) => (
                <button key={animal.id} type="button" onClick={() => setAnimalId(animal.id)} className={`flex items-center gap-3 rounded-md border p-3 text-left ${animalId === animal.id ? "border-teal-600 bg-teal-50" : "border-slate-custom-50"}`}>
          ...
          <button type="button" onClick={() => go(2)} className="w-fit rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white">Continuar</button>
        </div>
      </section>
        ...
        <section className="rounded-lg border border-slate-custom-50 bg-cream-50 p-5 shadow-sm">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {enabledMethods.map((option) => {
              ...
              return (
                <button key={option} type="button" onClick={() => setMethod(option)} className={`rounded-md border p-4 text-left ${method === option ? "border-teal-600 bg-teal-50" : "border-slate-custom-50"}`}>
          ...
          {selectedMethod && (
            <div className="mt-4 rounded-md border border-slate-custom-50 bg-cream-100 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm text-slate-custom-700">Escanea el QR o envía al número, luego sube comprobante.</p>
              ...
              {selectedMethod.qr_path && <img src={`${STORAGE}/${selectedMethod.qr_path}`} alt="QR de pago" className="mt-4 h-40 w-40 rounded-md border bg-cream-50 object-contain p-1" />}
            </div>
          )}
          {errors.method && <p className="mt-2 text-xs text-rose-600">{errors.method}</p>}
          <button type="button" onClick={() => go(3)} className="mt-4 w-fit rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white">Continuar</button>
        </section>
      )}
      ...
      {step >= 3 && (
        <section className="rounded-lg border border-slate-custom-50 bg-cream-50 p-5 shadow-sm">
          ...
          <div className="mt-4">
            {!file ? (
              <div onDrop={(e) => { e.preventDefault(); const next = e.dataTransfer.files[0]; if (next) pickFile(next); }} onDragOver={(e) => e.preventDefault()} onClick={() => fileRef.current?.click()} className="cursor-pointer rounded-md border-2 border-dashed border-slate-300 bg-cream-100 px-6 py-8 text-center text-sm text-slate-custom-700">
          ...
          <button disabled={loading} className="mt-5 rounded-md bg-brand-600 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">
