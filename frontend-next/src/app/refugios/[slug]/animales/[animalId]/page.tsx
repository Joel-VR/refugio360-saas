...
export default function RefugioAnimalDetailPage() {
  ...
  return (
    <SimplePage title={animal?.name ?? "Ficha del animal"} description={animal?.health_status ?? "Información pública del animal."}>
      <div className="rounded-lg border border-slate-custom-50 bg-cream-50 p-5">
        <p className="text-sm text-slate-custom-700">Especie: {animal?.species ?? "-"}</p>
        <p className="text-sm text-slate-custom-700">Estado: {animal?.lifecycle_status ?? "-"}</p>
        <Link href={`/login?next=/refugios/${slug}/animales/${animalId}/adoptar`} className="mt-4 inline-block rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white">
          Solicitar adopción
        </Link>
      </div>
    </SimplePage>
