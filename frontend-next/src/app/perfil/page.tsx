    ...
    <PublicShell>
      <section className="mx-auto grid max-w-4xl gap-6 px-6 py-10">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600">Administra tus datos de cuenta, contraseña y foto de perfil.</p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-[260px_1fr]">
          <section className="grid h-fit gap-4 rounded-lg border border-slate-200 bg-white p-5">
            <div className="mx-auto grid h-28 w-28 place-items-center overflow-hidden rounded-full border border-slate-200 bg-slate-50 text-3xl font-semibold text-teal-800">
            ...
            <label className="cursor-pointer rounded-md border border-slate-300 px-4 py-2 text-center text-sm font-semibold text-slate-700 hover:bg-slate-50">
          ...
          <div className="grid gap-6">
            <form onSubmit={saveProfile} className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5">
              ...
              <label className="grid gap-1 text-sm font-medium text-slate-700">
                Rol
                <input readOnly className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 font-normal text-slate-600" value={roleLabel(user?.role)} />
              </label>
              <button disabled={loadingProfile} className="w-fit rounded-md bg-teal-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-70">
                {loadingProfile ? "Guardando..." : "Guardar datos"}
              </button>
            </form>

            <form onSubmit={savePassword} className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5">
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-xl font-semibold">Cambiar contraseña</h2>
                <button type="button" onClick={() => setShowPasswords((value) => !value)} className="grid h-9 w-9 place-items-center rounded-md border border-slate-300 text-slate-600 hover:bg-slate-50" title={showPasswords ? "Ocultar contraseñas" : "Ver contraseñas"} aria-label={showPasswords ? "Ocultar contraseñas" : "Ver contraseñas"}>
                  <EyeIcon />
                </button>
              </div>
              ...
              <p className="text-sm text-slate-600">La contraseña debe tener mínimo 8 caracteres, mayúscula, minúscula, número y símbolo.</p>
              <button disabled={loadingPassword} className="w-fit rounded-md bg-teal-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-70">
