      ...
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        ...
        <div className="flex flex-col gap-4 rounded-3xl border border-slate-custom-50 bg-cream-50 p-6 shadow-sm md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={() => applyQuickRange("week")} className="rounded-full border border-slate-custom-50 bg-cream-100 px-3 py-1.5 text-xs text-slate-custom-700 hover:bg-slate-custom-50">Última semana</button>
            <button onClick={() => applyQuickRange("month")} className="rounded-full border border-slate-custom-50 bg-cream-100 px-3 py-1.5 text-xs text-slate-custom-700 hover:bg-slate-custom-50">Último mes</button>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              ...
              {!loading && donations.map((d) => (
                ...
                >
                  <td className="px-5 py-3 font-medium text-slate-custom-900">{d.donor_name || "Anónimo"}</td>
                  ...
                  <td className="px-5 py-3 font-mono text-xs text-slate-custom-400">{d.operation_reference || "—"}</td>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-8" onClick={onClose}>
        ...
        >
          <div className="mb-6 flex items-start justify-between">
            <div>
              <p className="text-xs uppercase tracking-widest text-brand-600">Detalle de donación</p>
              <h2 className="mt-1 text-2xl font-semibold text-slate-custom-900">{donation.donor_name || "Donante anónimo"}</h2>
            </div>
            <button onClick={onClose} className="text-slate-custom-400 hover:text-slate-custom-700">✕</button>
          </div>
