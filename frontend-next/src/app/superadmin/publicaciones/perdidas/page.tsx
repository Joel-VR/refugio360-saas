...
export default function SuperAdminLostPostsPage() {
  ...
  useEffect(() => {
    getSuperAdminLostFoundPosts("perdida")
      .then(setPosts)
  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-6 px-6 py-10">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-custom-900">Mascotas perdidas</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-custom-700">
          Aprueba o rechaza reportes de mascotas perdidas antes de que aparezcan públicamente.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <LostFoundReviewList
          initialPosts={posts}
          emptyText="No hay publicaciones de mascotas perdidas."
