...
export function ShelterAnimals({ slug }: { slug: string }) {
    ...
    return (
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="flex flex-col overflow-hidden rounded-2xl border border-slate-custom-50 bg-white p-4 animate-pulse">
          ...
          >
            <div>
              <div className="relative h-48 w-full overflow-hidden bg-slate-100 text-slate-300">
                ...
                <span className={`absolute right-3 top-3 rounded-full border px-2.5 py-0.5 text-[11px] font-semibold backdrop-blur-md ${badge.className}`}>
                  {badge.label}
                </span>
              </div>

              <div className="p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-brand-600">{animal.species}</p>
                <h3 className="mt-0.5 text-lg font-semibold text-slate-custom-900 group-hover:text-brand-600 transition">{animal.name}</h3>
              </div>
            </div>
